...
def load_calibrations(data_fp):
    ...
    for _data_drive in data_drives:
        ...
        for _data_id in _data_ids:
            ...
            #     print("Skip reading transform matrix again, since it is the same.")
            # print _data_id
            if data_info["T"] is None:
                data_info["T"] = transform_mat
            else:
                if not np.array_equal(data_info["T"],  transform_mat):
                    raise Exception("data_info[T] != transform_mat, please make sure the calibration is consistant")
            if (data_info["P"] is None) and (data_info["R"] is None):
                data_info["P"], data_info["R"] = load_cam_calibrations(_cam_mat_fp)
            else:
                P, R = load_cam_calibrations(_cam_mat_fp)
                if (not np.array_equal(P, data_info["P"])) or (not np.array_equal(R, data_info["R"])):
                    raise Exception("Please make sure the calibration is consistant")

    print("Valid calibration info has been loaded!")
    return data_info
